...
export default function AddCustomerPaymentModal({
  ...
  const remainingAmount = order.totalSaleAmount - order.receivedFromCustomer;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(null);
      await supplyOrderApi.addCustomerPayment(order._id!, {
        ...payment,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">Add Customer Payment</h2>
        ...
        <div className="bg-blue-50 text-blue-600 px-4 py-2 rounded-lg mb-4">
          <p>Customer: {order.customer.businessName || `${order.customer.firstName} ${order.customer.lastName}`}</p>
